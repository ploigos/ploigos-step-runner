...
|--------------------|-----------|-------------|-----------
| `fail-on-no-tests` | False     | True        | Value to specify whether unit-test
                                                 step can succeed when no tests are defined
| `pom-file`         | False     | `'pom.xml'` | pom used to run tests and check
                                                 for existence of custom reportsDirectory
| `selenium-hub-url` | True      |             | URL where the Selenium Hub is
| `target-base-url`  | True      |             | URL where the UAT application is running
